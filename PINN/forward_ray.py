...
def asd(config):
    ...
    for i in range(number_of_epoch):
        ...
        loss = (config['a'])*loss0 + (1-config['a'])*loss2 #+ loss3    # add two loss terms together
        loss.backward()
        optimizer.step()
        ...
        if (i+1) %100== 0: 
            ...
            session.report({"loss": (loss.item())})#, checkpoint=checkpoint) # ray
...
def main2(resource, num_samples, gpus_per_trial, limit):
    ray.init(num_gpus=resource[0], num_cpus=resource[1])
    config = {
        "a": tune.uniform(1e-60, 1)}
    bayesopt=BayesOptSearch(utility_kwargs={"kind": "ucb", "kappa": 1.5, "xi": 0.0})
    algo = ConcurrencyLimiter(bayesopt, max_concurrent=limit)
    scheduler = AsyncHyperBandScheduler(max_t=1e4)

    tuner = tune.Tuner(
        tune.with_resources(
            tune.with_parameters(asd),
...
N=16
main2(resource=[1,N], num_samples=10000, gpus_per_trial=1/N, limit=N) # ray
